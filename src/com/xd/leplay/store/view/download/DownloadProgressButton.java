...
public class DownloadProgressButton extends FrameLayout implements
		OnClickListener
{
	...
	private void initLayout()
	{
		...
		downloadbtn.setTextColor(Color.parseColor(AppStateConstants
				.getAppStateTextColor(appState)));
		downloadbtn.setGravity(Gravity.CENTER);
		downloadbtn.setText(AppStateConstants.getAppStateText(appState));
		downloadbtn.setBackgroundResource(AppStateConstants
				.getAppStateBackgroundResource(appState));
		downloadbtn.setPadding(0, 0, 0, 0);
		downloadbtn.setOnClickListener(this);
		downloadbtn.setTextSize(TypedValue.COMPLEX_UNIT_SP, 13);
	...
	{
		if (appState == AppStateConstants.APP_DOWNLOAD_STATE_STOP)
		{
		...
		} else if (appState == AppStateConstants.APP_DOWNLOAD_STATE_DOWNLOADING)
		{
			downloadpb.setVisibility(View.VISIBLE);
			pdrawable = context.getResources().getDrawable(
					R.drawable.progress_downloading_bg);
