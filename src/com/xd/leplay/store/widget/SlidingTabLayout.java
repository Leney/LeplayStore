...
public class SlidingTabLayout extends HorizontalScrollView
{
	...
	private ArrayList<TextView> tabTitleViews = null;
	private int screenX = 0;
	private int textViewWidth = 0;
	private TypedArray ta = null;
	private int nomalTextColor, selectedTextColor;
	...
	public SlidingTabLayout(Context context, AttributeSet attrs, int defStyle)
	{
		...
		ta = context.obtainStyledAttributes(attrs,
				com.xd.leplay.store.R.styleable.SlidingTabStrip);
		int bottomBackground = ta
				.getColor(
						com.xd.leplay.store.R.styleable.SlidingTabStrip_bottomBackground,
						0);
		int dividerColor = ta
				.getColor(
						com.xd.leplay.store.R.styleable.SlidingTabStrip_dividerColor,
						context.getResources()
								.getColor(
										com.xd.leplay.store.R.color.main_tab_bg_color));
		nomalTextColor = ta
				.getColor(
						com.xd.leplay.store.R.styleable.SlidingTabStrip_nomalTextColor,
						0);
		selectedTextColor = ta
				.getColor(
						com.xd.leplay.store.R.styleable.SlidingTabStrip_selectedTextColor,
						0);
		mTabStrip = new SlidingTabStrip(context);

		float titleTextSize = ta
				.getDimensionPixelSize(
						com.xd.leplay.store.R.styleable.SlidingTabStrip_titleTextSize,
						0)
				/ density;
		if (titleTextSize > 1)
		{
			TAB_VIEW_TEXT_SIZE_SP = (int) titleTextSize;
		}
		if (bottomBackground != 0)
		{
			mTabStrip.setBottomBackground(bottomBackground);
		}
		if (dividerColor != 0)
		{
	...
	public void setViewPager(ViewPager viewPager, int screenX)
	{
		...
		textViewWidth = (screenX / count);
		Log.i("9999", "textViewWidth-->" + textViewWidth);
	...
	protected TextView createDefaultTabView(Context context)
	{
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB)
		{
			...
			{
				textView.setBackgroundResource(customBackgroundId);
			}
		}
	...
	private void populateTabStrip()
	{
		...
		for (int i = 0; i < adapter.getCount(); i++)
		{
			...
			String content = (String) adapter.getPageTitle(i);
			textView.setText(content);
			int length = (int) (TextUtil.getTextViewLength(textView, content) + padding * 2);
			totalWidth += length;
			if (totalWidth > screenX - dx)
			{
				textViewWidth = 0;
				break;
			}
		...
		for (int i = 0; i < adapter.getCount(); i++)
		{
			...
			if (i == 0)
			{
				...
				{
					tabTitleView.setTextColor(selectedTextColor);
				}
			} else
			{
				if (nomalTextColor != 0)
				{
					tabTitleView.setTextColor(nomalTextColor);
				}
			}
			...
			if (count > 0)
			{
				for (int i = 0; i < count; i++)
				{
					...
					} else
					{
						if (nomalTextColor != 0)
						{
							tabTitleViews.get(i).setTextColor(nomalTextColor);
						}
					}
				}
			}
